...
new-module -name CIUtils -scriptblock {
  ...
  function Install-Utility {
    ...
    $InstallDir = "$env:SystemDrive\ci-utils"
    $SettingsDir = "$env:SystemDrive\ci-settings"

    $RemoteAsset = "https://packages.chef.io/files/$channel/ci-utils/latest/ci-utils-windows.tar.gz"
    ...
    $client.DownloadFile($RemoteAsset, "$env:SystemDrive\ci-utils.tar.gz")

    Write-Output "Extracting ci-utils into $InstallDir"
    & cmd.exe '/C 7z x "C:\ci-utils.tar.gz" -so | 7z x -aoa -si -ttar -o"C:\"'
    ...
    Remove-Item -Path "C:\ci-utils.tar.gz"
  }
